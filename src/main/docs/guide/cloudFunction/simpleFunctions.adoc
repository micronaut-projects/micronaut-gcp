...
snippet::example.background.Example[source="main", languages="java,kotlin,groovy"]
...
[.lang-python]
NOTE: A Google Cloud Function needs a Java entry point class that the Functions Framework instantiates before any Micronaut application context (and with it the Python runtime) exists, so raw functions cannot be written in Python. Use the HTTP functions described below, which run a regular Micronaut application, instead.
