...
[.lang-python]
NOTE: In Python the client is an abstract class (`ABC`) whose `@Topic` methods are abstract with `...` bodies; since Python has no method overloading, each message type gets its own method name.
