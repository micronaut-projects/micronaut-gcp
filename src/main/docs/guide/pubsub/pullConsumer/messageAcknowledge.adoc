The message delivery is handled by an `IntroductionAdvice` class api:intercept.PubSubConsumerAdvice[]. It invokes the method annotated with ann:configuration.gcp.pubsub.annotation.Subscription[].
