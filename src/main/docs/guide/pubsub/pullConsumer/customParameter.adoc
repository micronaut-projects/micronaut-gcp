...
[.lang-python]
NOTE: The Python binder implements the raw `PubSubAnnotatedArgumentBinder` interface and returns the generated annotation class from `getAnnotationType()`; the annotation itself is a decorator function meta-annotated with `@Bindable`.
